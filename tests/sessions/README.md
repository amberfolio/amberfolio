...
| `walk-map.rec` | the same | the same script with **`automap` on**: `Tab` is taken out of the keystroke buffer before the program's own key routine looks, and the panel is drawn into the EGA planes over the party roster, the streets filling in behind the party as it walks (M5-E2 #173). The seam is reached 1,283,230 times |
