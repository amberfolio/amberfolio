...
// loop. Everything M1 adds afterwards — the ALU flag kernel, the decoder,
// sixteen instruction families, interrupt delivery — hangs off this.
//
...
//
// Scope note: this header knows nothing about instruction encoding. Even
// the opcode fetch below is a byte read that happens to be at CS:IP — the
// decode of it arrives in M1-F3.
...
class processor {
 public:
  /// `machine_bus` and `log` must outlive the processor. `log` may be
  /// null: a stop is still recorded and still returned (diagnostics.h
  /// explains why that is not a hole in the "log, don't fake" rule).
  explicit processor(bus& machine_bus, diagnostics* log = nullptr) noexcept;
  ...
  /// Memory, addressed the way the program addresses it. Offsets are
  /// segment-relative and the pair is folded to a physical address by
  /// `physical_address` — including its 1 MiB wrap.
 ...
 private:
  ...
  /// the bottom of the same one, it does not roll into the next.
  std::uint8_t fetch_byte();

  /// Record the stop, tell the sink, rewind IP to `start_ip` so the
  /// record and the machine agree, and answer `stopped`.
  step_status stop_unimplemented(std::uint8_t opcode, std::uint16_t start_ip);
