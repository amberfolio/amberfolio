...
| `--wall now\|none\|YYYY-MM-DD[THH:MM[:SS[.CC]]]` | what date to tell the machine it is (#320). This host's own clock unless you say otherwise, read once before the first instruction and printed. The machine's date is a *seed* plus virtual time (`machine/platform.h`) and no host ever set one until #320, so the game — and every stamp in the journal's listing — read 1 January 1980 plus the run's uptime. `none` is that machine, kept: unseeded, which is what a PC with no clock card gave you and what every recording in `tests/sessions/` was made on. A **stated** date is for a run that has to be reproducible, and that is more than a hash: the seed is machine state, so it is in every checkpoint, and the program *reads* the date — 400 million steps of a real boot dumped at two instants a minute apart differ in 73 pixels, at the same step and the same tick (`docs/replay.md` §6 has the measurement). **Two runs compared with each other, by hash or by pixel, have to be told the same date** — `scripts/visual-legs.py` states `--wall none` on both of its sides for exactly this reason, and the same leg without that line fails on 178 pixels the seam does not own. The session library's `contrast` and `identical` pairs still owe theirs (#293). Recorded either way, as a `wall` line at the tick it was seeded at; refused alongside `--replay`, which takes its date from the recording. |
