...
**The bump rule, and the surface it is about (#375).** `major` moves when
an entry point is removed, renamed or changes meaning; `minor` when entry
points are added and nothing that was there changed. That surface is the
**module's**, not this header's alone: `hosts/web/CMakeLists.txt`'s own
`af_web_*` exports sit in the same `exports` block as `abi.h`'s functions,
a consumer calls both kinds without being able to tell them apart, and a
rename or removal of either moves these numbers — `abi.h` says so at
`AF_ABI_VERSION_MAJOR`'s own definition, in exactly those words. A loader
...
joins the pieces (`docs/journal.md` §5), 1.4 added the ten
`af_machine_save_layer_*` calls (#208), which are where a host learns
which of the files on the machine's filesystem are the player's (§6), 1.5
added the eighteen `af_screen_keyboard_*` calls (#377), which are the
layouts and the navigation model of the keyboard both hosts paint on the
screen (§7), and 2.0 corrects a bump 1.3 owed and did not take:
`v0.5.0` renamed `af_web_automap_store` to `af_web_save_sidecars` (#351),
...
newline-joined so a harmless reordering of the CMake block does not move
it, generated by `release-bundle.sh` from the same list `exports` is —
never a second one kept in step by hand. `major`/`minor` says whether a
loader *should* still work; `exportsDigest` says whether the surface it
is about to fetch bytes for is the surface a pinned digest names, catching
an add, a remove or a rename in one comparison, before a byte of the
module is fetched. It would have caught #375 by itself: 1.3's export
count went *up* by thirteen the same release a name left, which is
exactly what a count cannot see and a digest of the names does.
...
declaration", never as 1.0. `exportsDigest` carries no such exemption —
it is generated from the export list alone, which every tree this script
stages has.
